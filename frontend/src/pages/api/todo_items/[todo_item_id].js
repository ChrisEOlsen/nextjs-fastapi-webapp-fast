...
async function handlePut(req, res, todo_item_id) {
  try {
    ...
    if (!backendResponse.ok) {
      return res.status(backendResponse.status).json({ error: data.detail || 'Failed to update todo_item' });
    }
...
async function handleDelete(req, res, todo_item_id) {
  try {
    ...
    if (!backendResponse.ok) {
      ...
      return res.status(backendResponse.status).json({ error: data.detail || 'Failed to delete todo_item' });
    }
